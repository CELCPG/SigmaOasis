...
describe('the action row is wired to the answer', () => {
  ...
  test('the row is gated by actionsReady, not by the streaming flag', () => {
    assert.match(source, /const canAct = actionsReady\(\{/)
    assert.ok(source.includes('{canAct && ('), 'the action row must open on canAct')
    assert.ok(
  ...
  test('no button inside the row re-introduces the streaming gate', () => {
    const start = source.indexOf('{canAct && (')
    ...
    for (const affordance of ['📋 Copy', '↻ Regenerate', '🧠 Think harder', '<BranchMenu', 'formatTime(message.createdAt)']) {
      assert.ok(row.includes(affordance), `${affordance} must live in the row that canAct opens`)
    }
