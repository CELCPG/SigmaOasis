 ...
 *
 * Provenance, because it matters for a file that decides what a judge reads:
 * this generator and the r9 documents it wrote were committed on the release
 * line (2c669e3) and then dropped by the round-13 merge (f8cf9dc), whose second
 * parent had never carried them — the merge took that side's tree. Rounds 10–13
 * were judged from documents that are not in the repository. Restored for
 * round 14 from the original commit, with one addition: the COUNTS block that
 * critic-counts.mjs introduced in round 10 now rides in the output shape, read
...
// The machine-readable block a critic fills in beside the prose (round 10+),
// printed by the script that also reads it back, so the spec in front of the
// critic is the spec the transcriber enforces.
const countsBlock = execFileSync(process.execPath, [join(here, 'critic-counts.mjs'), 'block'], {
  encoding: 'utf8'
}).trimEnd()
    ...
    [
      ...
      wrap(
        'If the WINNER is `tie`, BIGGEST REMAINING GAP is the biggest thing BOTH runs get ' +
          `wrong. Produce the ${ids.length} blocks in the order below, each followed by its ` +
          'COUNTS block, and nothing else.'
      ),
      '',
      countsBlock
    ].join('\n')
