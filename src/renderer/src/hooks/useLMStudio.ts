...
async function runTurn(
  ...
  const checkGrounding = async (): Promise<void> => {
    ...
    // Same question, same rule: the revision that came back is the evidence it
    // ran. `reviseAgainstFindings` returns '' when the loop produced nothing —
    // including when the deadline cut it off mid-stream.
