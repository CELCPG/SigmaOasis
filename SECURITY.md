...
Include reproduction steps and the OpenMind version. Expect an initial response within a week.

## Security model

OpenMind runs local models with **agentic tools** — file access, a shell, and a local memory store.
...
OpenMind talks to your local LM Studio server (loopback only — enforced by the renderer's CSP).
...
codesign --verify --deep --strict --verbose=2 /Applications/OpenMind.app
```
